...
<script>
  ...
  export default {
    ...
    computed: {
      data2(){
        //alert(this.$store.state.roleFun)
//        if (this.$store.state.roleFun.length==0) {
//          return this.ajaxData;
//        }
//        return this.$store.state.roleFun
      ...
      checkednode(){
        ...
        vm.$nextTick(function () {
          ...
          let n = [];
          let c = forEachChild(n, vm.$store.state.roleFun);
          console.log(JSON.stringify(c))
          if (v > 0) {
            child = 2
            //this.$refs.tree.setCheckedKeys([30,28,33,34,40])
    ...
    mounted(){
      this.$ajax.post('/node/funlist').then(response => {
        ...
        //console.log('sureNode', JSON.stringify(response.data.data.list));
        // this.ajaxData = response.data.data.list;
        allNodeId = [];
  ...
  function forEachChild(child, arr) {
    ...
    arr.forEach(function (value, index, self) {
      if (value.children.length > 0 && value.node_status == 1) {
        forEachChild(child, value.children);
      }
      if (value.node_status == 1 && value.children.length ==0) {
        child.push(value.id);
