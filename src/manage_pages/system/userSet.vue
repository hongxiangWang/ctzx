    ...
    mounted(){
      this.$ajax.post('user/getuserlist')
        .then((respons) => {
          ...
          let _this = this;
          axajData.forEach(function (value, index, sef) {
            //value.status == "1" ? value.status = '正常' : value.status = '异常';
            value.lock == "0" ? value.lock = '未锁' : value.lock = '已锁';
            if (value.status == "1") {
              value.status = '正常'
              _this.tableData.push(value);
            }
          })
          axajData = _this.tableData;
          this.rowTableStyle = (row, index) => {
            if (row.status === '非正常') {
              return 'info-row';
            } else if (row.lock === '已锁') {
              return 'positive-row';
